    ...
    <View style={{ flex: 1, backgroundColor: 'white' }}>
      <SafeAreaView style={{ padding: 16, flex: 1 }}>
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 24 }}>
          ...
          <Text style={{ fontSize: 20, fontWeight: '600' }}>Pagamento concluído</Text>
        </View>

        <View style={{ alignItems: 'center', marginTop: 40 }}>
          <Icon name="check-circle" type="material" size={80} color="#10B981" />
          <Text style={{ fontSize: 18, fontWeight: '600', marginTop: 20 }}>Pagamento realizado com sucesso!</Text>
          {session_id && (
            <Text style={{ marginTop: 12, color: '#6B7280' }}>Sessão: {String(session_id)}</Text>
          )}
          ...
          <View style={{ marginTop: 32, width: '100%' }}>
            <TouchableOpacity onPress={() => router.push('/compras')} style={{ backgroundColor: '#2563EB', padding: 12, borderRadius: 8, alignItems: 'center' }}>
              <Text style={{ color: 'white', fontWeight: '600' }}>Ver minhas compras</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={() => router.replace('/')} style={{ marginTop: 12, padding: 12, borderRadius: 8, alignItems: 'center', borderWidth: 1, borderColor: '#E5E7EB' }}>
              <Text style={{ color: '#374151', fontWeight: '600' }}>Voltar para o início</Text>
            </TouchableOpacity>
          </View>
        </View>
      </SafeAreaView>
